...
from pcaps.constants import E_LINK_TYPE, E_PROTOCOL
...
class PcapBodyFactory:
    ...
    @classmethod
    def register(cls, link_type: int, protocol: int, body_cls: type[abPcapBody]) -> None:
        """확장 — 새 link-type/protocol 조합에 대한 body 클래스 등록."""
        cls._BODY_TABLE[(link_type, protocol)] = body_cls

    @classmethod
    def parse(cls, link_type: int, pak_len: int, file_point: BinaryIO) -> Optional[abPcapBody]:
